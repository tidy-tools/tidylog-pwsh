...
if (ShouldRun "WAIT") {
	if (-not $NoWait) {
		...
		Write-TLPhase "WAIT" "Blocking functions that display ""·"" during the wait."
			...
			Wait-TLTimed -Label "-Seconds 4" -Seconds 4
		Start-Sleep -Milliseconds 500

			Write-TLDetail "Wait-TLTimed - 8 seconds and above (colour stages)" -BeginSection
			Wait-TLTimed -Label "-Seconds 8" -Seconds 8
			Wait-TLTimed -Label "-Tone Warm" -Seconds 12 -CompletionMessage "-CompletionMessage" -Tone Warm
		Start-Sleep -Milliseconds 500
			...
			$result = Wait-TLConditional -Label "Waiting for file" -Condition { Test-Path "$env:TEMP\tl-demo-flag.txt" } -TimeoutSec 10
			...
			$result = Wait-TLConditional -Label 'Will timeout: -TimeoutSec 5 -Message "service did not respond"' -Condition { $false } -TimeoutSec 5 -TimeoutMessage "service did not respond"
...
if (ShouldRun "INPUT") {
	if (-not $NoInput) {
		...
		Write-TLPhase "SELECTION" "Read-TLSelection - selection menus"
			...
			Write-TLDetail "Received" $choice.Index
			...
			Write-TLDetail "Received" $choice.Key
	} else {
